...
router.patch('/:id/score', async (req, res, next) => {
  try {
    ...
    const match = await Match.findById(req.params.id)
      .populate('requester', 'name email elo matchesPlayed clerkId')
      .populate('opponent',  'name email elo matchesPlayed clerkId');
    ...
    if (bothConfirmed) {
      ...
      // Use clerkId consistently as the PostHog distinctId across every
      // server-side event (matches what the client identifies with in
      // AuthContext.tsx), so a user's events form one timeline.
      analytics.eloUpdated(winner.clerkId, { newElo: newWinnerElo, delta: winnerDelta });
      analytics.eloUpdated(loser.clerkId,  { newElo: newLoserElo,  delta: loserDelta  });
    }
